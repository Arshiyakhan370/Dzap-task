      ...
      if (parts.length === 2) {
        ...
        if (!/^0x[0-9a-fA-F]{40}$/.test(address)) {
          ...
          if (newAddressValueMap.has(address)) {
            const originalLineNo = duplicateLines.get(address);
            errors.push(`Address ${address} encountered duplicates in line ${originalLineNo} and ${i + 1}`);
          } else {
            newAddressValueMap.set(address, intValue);
            validAddresses.push({ address, amount: intValue });
            duplicateLines.set(address, i + 1);
          }
        }
      } else {
