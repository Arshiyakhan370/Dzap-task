...
class Disperse extends Component {
  ...
  onSubmit = () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (parts.length !== 2) {
        errors.push(`Line ${i + 1}: Invalid format. It should be: "address value"`);
      } else {
        ...
        if (!/^-?\d+$/.test(value)) {
          errors.push(`Line ${i + 1}: Invalid value. It should be an integer.`);
        }

        // Store the address and value
        addressValueMap.set(address, parseInt(value));
      }
    }
    ...
    if (errors.length > 0) {
      ...
      this.setState({ validInput: true, error: null, addressValueMap });
    }
  };

  render() {
    const { inputText, error, validInput, addressValueMap } = this.state;
    ...
    return (
      <div className="container" style={{ width: '60vw' }}>
        ...
        {validInput && (
          <div className="mt-3">
            <h4>Processed Address-Value Pairs:</h4>
            <ul>
